...
Swift 1.2 允许通过多值并行 optional 绑定来避免 `if let` 语句的多重深嵌套。多个 optional 绑定可以通过逗号分隔开，并且可以带一个和传统的 `if` 语句同样效果的 `where` 从句。这样的话，拜占庭的 [厄运金字塔](http://www.scottlogic.com/blog/2014/12/08/swift-optional-pyramids-of-doom.html) 就可以被修缮成中世纪风格的现代牧场了：
...
> 这两个例子中判断语句的执行顺序是一样的。使用新的语法可以让每个绑定都按顺序执行，如果某一个绑定是 `nil` 就会停下来。
