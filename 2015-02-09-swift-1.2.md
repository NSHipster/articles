...
Swift 1.2 allows multiple simultaneous optional bindings, providing an escape from the trap of needing multiple nested `if let` statements to unwrap multiple values. Multiple optional bindings are separated by commas and can be paired by a `where` clause that acts like the expression in a traditional `if` statement. As such, the byzantine [pyramid of doom](http://www.scottlogic.com/blog/2014/12/08/swift-optional-pyramids-of-doom.html) has been renovated into a mid-century modern ranch:
