...
NSHipster has frequently gushed about how great Cocoa date & time calculation APIs are. iOS 7 makes them _even better_ by adding a couple of useful methods:

~~~{objective-c}
BOOL isToday = [calendar isDateInToday:date];
BOOL isWeekend = [calendar isDateInWeekend:date];
BOOL isSameMonth = [calendar isDate:date1
                        equalToDate:date2
