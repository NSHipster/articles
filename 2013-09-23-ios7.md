...
let timer = NSTimer(
    timeInterval: 1.0,
    target: self,
    selector: "incrementCompletedUnitCount:",
    userInfo: nil,
    repeats: true
)
...
println("First Object: \(array.firstObject)") // Optional(1)
println("Last Object: \(array.lastObject)") // Optional(3)
~~~
...
let features = smileDetector.featuresInImage(image, options: [CIDetectorSmile : true])
if let feature = features.first as? CIFaceFeature where feature.hasSmile {
    UIImageWriteToSavedPhotosAlbum(image, self, "didFinishWritingImage", features)
} else {
