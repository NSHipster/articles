...
From an empiricist perspective of the universe, two objects are equal if they are indistinguishable from one another in measured observations. On a human scale, egalitarians hold that individuals should be considered equal members of the societal, economic, political, and judicial they inhabit.
...
Here's an idea of how `NSArray` might do this:
...
Interestingly enough, Objective-C selector names are also stored as interned strings in a shared string pool. `themoreyouknow.gif`. 
...
- (NSUInteger)hash {
  ...
  return ((NSUInteger)red << 16) + ((NSUInteger)green << 8) + blue;
}
@end
~~~
...
- (BOOL)isEqualToPerson:(Person *)person {
  ...
  return [self.name isEqualToString:person.name] && [self.birthday isEqualToDate:person.birthday];
}
...
- (BOOL)isEqual:(id)object {
  ...
  if (![[object class] isKindOfClass:[Person class]]) {
    return NO;
  }
